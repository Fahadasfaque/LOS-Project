...
import { Search, RefreshCw, ChevronLeft, ChevronRight, Eye, ShieldAlert, Download, Filter, FileText, AlertTriangle } from 'lucide-react';
    ...
    <div className="space-y-6 max-w-7xl mx-auto pb-10">
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          ...
          <p className="text-sm text-muted-foreground mt-1">Underwrite loan applications, evaluate documents, and transition statuses.</p>
        </div>
            ...
            <Table>
              <TableHeader className="bg-muted/50 border-b border-border sticky top-0 z-10 shadow-sm backdrop-blur-md">
                <TableRow className="hover:bg-transparent">
                  <TableHead className="text-muted-foreground font-semibold text-xs uppercase tracking-wider px-6 py-4">Application No.</TableHead>
                  <TableHead className="text-muted-foreground font-semibold text-xs uppercase tracking-wider px-6 py-4">Applicant Name</TableHead>
                  <TableHead className="text-muted-foreground font-semibold text-xs uppercase tracking-wider px-6 py-4">Loan Type</TableHead>
                  <TableHead className="text-muted-foreground font-semibold text-xs uppercase tracking-wider px-6 py-4 text-right">Loan Amount</TableHead>
                  <TableHead className="text-muted-foreground font-semibold text-xs uppercase tracking-wider px-6 py-4 text-right">Monthly Income</TableHead>
                  <TableHead className="text-muted-foreground font-semibold text-xs uppercase tracking-wider px-6 py-4">Status</TableHead>
                  <TableHead className="text-muted-foreground font-semibold text-xs uppercase tracking-wider px-6 py-4">Created At</TableHead>
                  <TableHead className="text-muted-foreground font-semibold text-xs uppercase tracking-wider px-6 py-4 text-right">Actions</TableHead>
                </TableRow>
              </TableHeader>
              <TableBody>
                {loading ? (
                  Array.from({ length: limit }).map((_, i) => (
                    <TableRow key={`skeleton-${i}`} className="border-b border-border">
                      <TableCell className="px-6 py-4"><div className="h-4 w-24 bg-muted rounded animate-pulse" /></TableCell>
                      <TableCell className="px-6 py-4"><div className="h-4 w-32 bg-muted rounded animate-pulse" /></TableCell>
                      <TableCell className="px-6 py-4"><div className="h-4 w-20 bg-muted rounded animate-pulse" /></TableCell>
                      <TableCell className="px-6 py-4 flex justify-end"><div className="h-4 w-24 bg-muted rounded animate-pulse" /></TableCell>
                      <TableCell className="px-6 py-4"><div className="h-4 w-24 bg-muted rounded animate-pulse ml-auto" /></TableCell>
                      <TableCell className="px-6 py-4"><div className="h-5 w-20 bg-muted rounded-full animate-pulse" /></TableCell>
                      <TableCell className="px-6 py-4"><div className="h-4 w-24 bg-muted rounded animate-pulse" /></TableCell>
                      <TableCell className="px-6 py-4"><div className="h-8 w-16 bg-muted rounded animate-pulse ml-auto" /></TableCell>
                    </TableRow>
                  ))
                ) : items.length === 0 ? (
                  <TableRow>
                  ...
                  items.map((item) => (
                    <TableRow key={item.id} className="border-b border-border hover:bg-muted/30 transition-colors duration-150 group">
                      <TableCell className="font-mono text-xs font-semibold text-primary px-6 py-4">{item.applicationNumber}</TableCell>
                      <TableCell className="font-semibold text-foreground px-6 py-4 text-sm">{item.applicantName}</TableCell>
                      <TableCell className="text-xs tracking-wider uppercase font-semibold text-muted-foreground px-6 py-4">{item.loanType}</TableCell>
                      <TableCell className="text-right font-bold text-foreground px-6 py-4">{formatCurrency(item.loanAmount)}</TableCell>
                      <TableCell className="text-right text-muted-foreground px-6 py-4 font-medium">{formatCurrency(item.monthlyIncome)}</TableCell>
                      <TableCell className="px-6 py-4">{getStatusBadge(item.status)}</TableCell>
                      <TableCell className="text-muted-foreground text-xs font-mono px-6 py-4">{new Date(item.createdAt).toLocaleDateString()}</TableCell>
