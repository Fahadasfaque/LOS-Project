...
import { FilePlus, Search, RefreshCw, ChevronLeft, ChevronRight, Eye, Download, Filter, MoreHorizontal, AlertTriangle } from 'lucide-react';
...
export default function MyApplicationsPage() {
  ...
  const getStatusBadge = (statusStr: string) => {
    switch (statusStr) {
      ...
      case 'UNDER_REVIEW':
        return <span className="inline-flex items-center rounded-md bg-amber-500/10 text-amber-600 px-2.5 py-1 text-xs font-semibold border border-amber-500/20 dark:text-amber-500">Reviewing</span>;
      case 'APPROVED':
    ...
    <div className="space-y-6 max-w-7xl mx-auto pb-10">
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          ...
          <p className="text-sm text-muted-foreground mt-1">Manage and track your initiated loan cases.</p>
        </div>
        <div className="flex gap-2">
          ...
          <Link href="/dashboard/create-application">
            <Button className="bg-primary hover:bg-primary/90 text-white font-semibold flex items-center gap-2 cursor-pointer h-10 px-4 rounded-md shadow-sm transition-all duration-150">
        ...
        <CardHeader className="p-4 border-b border-border bg-muted/20">
          <form onSubmit={handleSearchSubmit} className="flex flex-col lg:flex-row gap-4 items-end">
            <div className="flex-1 w-full space-y-1.5">
              <label className="text-xs font-semibold uppercase tracking-wider text-muted-foreground">Search Records</label>
                  ...
                  <TableRow>
                    <TableCell colSpan={8} className="py-20 text-center text-muted-foreground">
                      <div className="flex flex-col items-center justify-center">
                        ...
                        <p className="text-sm mt-1 max-w-sm">There are no applications matching your current filter criteria. Try adjusting your search or start a new application.</p>
                        <Button 
                          variant="outline" 
